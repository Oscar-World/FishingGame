...
class MainModel(val preferences: SharedPreferences) {
    ...
    */
    ...
    서버 통신하여 닉네임 중복 확인.
    */
    fun duplicateInputNickname(nickname: String) : Call<String> {

        return apiInterface.duplicateNickname(nickname)

    } // duplicateInputNickname()


    /*
    SharedPreference에 닉네임 저장 여부 반환.
     */
    ...
    닉네임 정규식.
     */
    fun isCheckNickname(nickname: String) : Boolean{
        ...
        // 조건에 다맞다면
        if(ps.matcher(nickname).matches() && nickname.trim() != "" && !nickname.contains(" ")){

            var isBadWord = false

            // 여기서 일치하는게 있는지 확인한다.
            for (i:Int in 0 until badWord.size){

                if( nickname.contains(badWord[i])){
   ...
   */
   ...
   도감 카운트
   */
    fun collectionCount(collectionItemList : ArrayList<Fish>) : Int {

        var count = 0

        for (i : Int in 0 until collectionItemList.size) {

            if (collectionItemList[i].name !="") {
    ...
    닉네임 포함 금지 단어.
     */
    val badWord = arrayOf<String>("ㅅㅂ", "씨발", "씨바", "개세끼", "18년", "18놈", "18새끼", "ㄱㅐㅅㅐㄲl", "ㄱㅐㅈㅏ", "가슴만져", "가슴빨아", "가슴빨어", "가슴조물락", "가슴주물럭", "가슴쪼물딱",
